    ...
    switch (infoFromRecipe[0]) {
      case('grams'):
        if (parseInt(infoFromRecipe[1])>100) {
          ponderateGreenScore = 1 + 0.01 * Math.exp(parseInt(infoFromRecipe[1])/100);
        } else {
      ...
      case('soupSpoon'):
        if (parseInt(infoFromRecipe[1]) * 15 >100) { /* a soup spoon weight 15g */
          ponderateGreenScore = 1 + 0.01 * parseInt(infoFromRecipe[1]) * 15/100;
        } else {
      ...
      case('coffeeSpoon'):
        if (parseInt(infoFromRecipe[1]) * 5 >100) { /* a coffee spoon weight 5g */
          ponderateGreenScore = 1 + 0.01 * parseInt(infoFromRecipe[1]) * 5/100;
        } else {
      ...
      case('pincee'):
        if (parseInt(infoFromRecipe[1]) * 0.5 >100) { /* a coffee spoon weight 0.5g */
          ponderateGreenScore = 1 + 0.01 * parseInt(infoFromRecipe[1]) * 0.5/100;
        } else {
